...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            dict(z0=0.0, dz=1.0, nlayers=3, ground_margin=0.1),
            [1, 2, 1],
            [5, 8, 9],
            [0.0, 1.0, 2.0],
        ),
...
def test_output_length_matches_n_strata(h_abg, nlayers, expected_n_strata):
    ...
    ni, n, min_layer = compute_ni_n(h_abg, veg_gnd, z0=0.0, dz=1.0, nlayers=nlayers, ground_margin=0.1)
...
def test_n_is_non_decreasing():
    ...
    _, n, _ = compute_ni_n(h_abg, veg_gnd, z0=0.0, dz=1.0, nlayers=3, ground_margin=0.1)
...
def test_ni_does_not_exceed_n_per_stratum():
    ...
    ni, n, _ = compute_ni_n(h_abg, veg_gnd, z0=0.0, dz=1.0, nlayers=5, ground_margin=0.1)
    assert np.all(ni <= n)
...
def test_output_dtypes():
    ...
    ni, n, min_layer = compute_ni_n(h_abg, veg_gnd, z0=0.0, dz=1.0, nlayers=3, ground_margin=0.1)
