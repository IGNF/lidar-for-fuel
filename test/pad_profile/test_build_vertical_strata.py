...
def test_breaks_starts_with_neginf():
    breaks, _ = build_vertical_strata(z0=0.0, dz=1.0, nlayers=60, ground_margin=0.1, h_abg=None)
    assert breaks[0] == -np.inf


def test_min_layer_first_element_is_neginf():
    _, min_layer = build_vertical_strata(z0=0.0, dz=1.0, nlayers=60, ground_margin=0.1, h_abg=None)
...
def test_returns_independent_arrays():
    breaks, min_layer = build_vertical_strata(z0=0.0, dz=1.0, nlayers=60, ground_margin=0.1, h_abg=None)
...
def test_breaks_shape_and_top(z0, dz, nlayers, expected_n_breaks, expected_n_min_layer, expected_last_break):
    breaks, min_layer = build_vertical_strata(z0=z0, dz=dz, nlayers=nlayers, ground_margin=0.1, h_abg=None)
...
def test_ground_margin_shift(z0, ground_margin, expected_break_1):
    breaks, min_layer = build_vertical_strata(z0=z0, dz=1.0, nlayers=10, ground_margin=ground_margin, h_abg=None)
...
def test_nlayers_none_derives_top_from_h_abg(h_abg, dz, expected_last_break):
    breaks, _ = build_vertical_strata(z0=0.0, dz=dz, nlayers=None, ground_margin=0.1, h_abg=h_abg)
    assert breaks[-1] == expected_last_break
...
def test_nlayers_none_requires_nonempty_h_abg(h_abg):
    with pytest.raises(ValueError, match="h_abg"):
        build_vertical_strata(z0=0.0, dz=1.0, nlayers=None, ground_margin=0.1, h_abg=h_abg)
