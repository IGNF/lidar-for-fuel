...
def _make_points(n: int, flight_agl: float = 1500.0, horizontal_offset: float = 0.0):
    ...
    return X, Y, Zref, Easting, Northing, Elevation
...
def test_compute_Nz_U_scanning_angle_false_returns_one():
    ...
    X, Y, Zref, E, N, Elev = _make_points(10, flight_agl=500.0)
    assert Nz_U(X, Y, Zref, E, N, Elev, scanning_angle=False) == 1.0


def test_compute_Nz_U_low_flight_height_returns_none():
    """Mean flight height below limit_flight_agl must return None."""
    X, Y, Zref, E, N, Elev = _make_points(10, flight_agl=500.0)
    result = Nz_U(X, Y, Zref, E, N, Elev, limit_flight_agl=800.0)
...
def test_compute_Nz_U_vertical_pulses_is_one():
    ...
    X, Y, Zref, E, N, Elev = _make_points(50, flight_agl=1500.0, horizontal_offset=0.0)
    result = Nz_U(X, Y, Zref, E, N, Elev)
...
def test_compute_Nz_U_oblique_angle_45_degrees():
    ...
    X, Y, Zref, E, N, Elev = _make_points(
...
def test_compute_Nz_U_output_shape():
    ...
    n = 37
    X, Y, Zref, E, N, Elev = _make_points(n, flight_agl=1500.0)
    result = Nz_U(X, Y, Zref, E, N, Elev)
    assert result is not None
...
def test_compute_Nz_U_values_in_0_1():
    ...
    X, Y, Zref, E, N, Elev = _make_points(100, flight_agl=1500.0, horizontal_offset=500.0)
    result = Nz_U(X, Y, Zref, E, N, Elev)
    assert result is not None
...
def test_compute_Nz_U_on_real_data():
    ...
    result = Nz_U(X, Y, Zref, Easting, Northing, Elevation)
