...
def filter_points_by_date(pdal_pipeline, deviation_day=1):
    """
    Creates a PDAL pipeline to filter a LiDAR point cloud,
    keeping only acquisitions within the densest ±x day window.

    Args:
        pdal_pipeline: Executed PDAL Pipeline object.
        deviation_day: Number of days around the main acquisition day.

    Returns:
        pdal.Pipeline: Configured PDAL pipeline for filtering points.
    """
    arrays = pdal_pipeline.arrays
    if len(arrays) == 0:
