"""
Compute the vertical height above ground (h_abg = Z - Z_sol) for each LiDAR point
...
Interpolation strategy:
    ...
    - NoData fallback: points on NoData pixels or outside the DTM extent
      receive h_abg = nodata_value (default: -9999).
"""
import logging
...
def filter_z_by_height(
    ...
    Args:
        ...
        min_height_filter (float): Minimum height (in metres) to remove noise points below the ground.
                                    Points with h_abg > height_filter are removed. Default: -3.
        height_filter (float): Height limit (in metres) to remove noise points above
            the canopy. Points with h_abg > height_filter are removed. Default: 80.

    Returns:
        np.ndarray: Filtered structured array with h_abg (float64) and filtered added as an
            extra field.
    """
    # Remove points too low (<-3) or too high (>height_filter m)
    mask = (points["h_abg"] >= min_height_filter) & (points["h_abg"] <= height_filter)
    n_removed = int((~mask).sum())
    if n_removed:
        logger.debug("%d points removed by h_abg filter [%s, %s]", n_removed, min_height_filter, height_filter)
    points = points[mask]
...
def add_h_abg(
    ...
    """
    Add h_abg = Z - Z_sol to each LiDAR point using bilinear interpolation on a DTM.
    ...
    fall on NoData pixels or outside the DTM extent receive h_abg = nodata_value.
    ...
    Args:
        ...
        nodata_value (float): Value assigned to h_abg for points on NoData DTM pixels
            or outside the DTM extent. Default: -9999 (from config dtm.nodata_value).

    Returns:
        np.ndarray: Filtered structured array with h_abg (float64) added as an
            extra field.
    """
    ...
    h_abg = (points["Z"] - z_ground).astype(np.float64)

    # Assign nodata_value to points on NoData DTM pixels or outside DTM extent
    nan_mask = np.isnan(h_abg)
    if np.any(nan_mask):
        logger.debug(
            "%d points on nodata or outside DTM extent — h_abg set to %s",
            int(nan_mask.sum()),
            nodata_value,
        )
        h_abg[nan_mask] = nodata_value
    logger.debug("h_abg — min: %.2f m, max: %.2f m, mean: %.2f m", h_abg.min(), h_abg.max(), h_abg.mean())

    # Append h_abg as extra dimension
    points_with_h_abg = rfn.append_fields(points, "h_abg", h_abg, dtypes=np.float64, usemask=False)

    return points_with_h_abg
